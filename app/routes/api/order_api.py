    ...
    try:
        ...
        for i, (seller_id, seller_data) in enumerate(seller_items.items()):
            ...
            try:
                ...
                # Rollback any created orders if there's an error
        ...
        for cart_item in cart_items:
            product = cart_item.product_id
            stats = ProductPurchaseStats.objects(product_id=product.id).first()
            if not stats:
                stats = ProductPurchaseStats(
                    product_id=product.id,
                    product_name=product.name,
                    purchase_count=0
                )
            stats.purchase_count += cart_item.quantity
            stats.last_purchased_at = datetime.utcnow()
            stats.save()
